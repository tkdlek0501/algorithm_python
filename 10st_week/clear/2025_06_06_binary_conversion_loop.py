...
# s가 1이 될 때까지 계속해서 이진변환하고
# 이진 변환 횟수와 그 과정에서 제거된 모든 0의 개수를 배열에 담아 return

def solution(s): # s는 0, 1 문자열
    count = 0
    zero_count = 0

    while s != "1":
        zero_count += s.count('0') # 0의 개수 count
        s = s.replace('0', '') # 0 제거
        s = bin(len(s))[2:] # 이진변환 이후 앞에 붙는 '0b' 잘라내기
        count += 1
