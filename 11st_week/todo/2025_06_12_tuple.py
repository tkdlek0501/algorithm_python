...
# <문제분석>
# 셀 수 있는 수량의 순서있는 열거 또는 어떤 순서를 따르는 요소들의 모음 튜플
# n개의 요소가 있으면 n-튜플

# (a1, a2, a3, ..., an)
# 1. 중복된 원소가 있을 수 있다. ex. (2,3,1,2)
# 2. 원소에 정해진 순서가 있어서 순서다르면 다른 튜플
# 3. 튜플의 원소 개수는 유한하다

# 원소의 개수 n개이고 '중복되는 원소가 없는' 튜플이면
# {{a1}, {a1, a2}, {a1, a2, a3} ...}

# ex. (2, 1, 3, 4)
# -> {{2}, {2, 1}, {2, 1, 3}, {2, 1, 3, 4}}
# 집합은 순서가 바뀌어도 상관 없어서 위 순서가 바뀌어도 괜찮다

# 특정 튜플을 표현하는 집합이 담긴 문자열 s가 주어지면
...
def solution(s):
    ...
    # 2. 각 요소를 숫자 리스트로 변환
    parsed_sets = [list(map(int, e.split(','))) for e in elements]
